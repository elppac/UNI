...
const toDisplayString = (val) => {
  return isString(val) ? val : val == null ? "" : isArray(val) || isObject$1(val) && (val.toString === objectToString$1 || !isFunction(val.toString)) ? JSON.stringify(val, replacer, 2) : String(val);
};
...
TokenStream.prototype.unescape = function(v) {
  ...
  var buffer2 = v.substring(0, index2);
  while (index2 >= 0) {
    ...
    switch (c) {
      ...
      case "u":
        ...
        buffer2 += String.fromCharCode(parseInt(codePoint, 16));
        index2 += 4;
        break;
      default:
    ...
    buffer2 += v.substring(index2, backslash < 0 ? v.length : backslash);
    index2 = backslash;
  }
  return buffer2;
};
